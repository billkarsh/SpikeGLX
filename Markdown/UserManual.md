...
Click `Run` in the `Configure Acquisition` dialog and the `Graphs Window`
opens automatically. A second Graphs Window can also be opened with menu
command `Window::More Traces`. Each windows shows up to two streams.

Here you can:

* Start and stop recording.
* View traces from any streams in the run.
* Apply a variety of filters.
* Open the ShankViewers.
* Open the SpikeViewers.
* Listen to traces.

There's a new [Help](Graphs_Help.html) button in the lower-right corner. 
...
You can open and view data files from any stream, and you can link the
files from a run so scrolling is synchronized between multiple viewers.
