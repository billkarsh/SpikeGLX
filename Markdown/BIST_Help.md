...
TCP/IP interface. The following subtests are performed:

+ FPGA Dev board Ethernet PHY access.
+ FPGA Dev board Ethernet TCP/IP test.
+ FPGA Dev board built-in self test (Kintex-7 FPGA, memory, peripherals).
+ FPGA Dev board configuration test.
...
electrode. The function analyses if the probe performance falls inside a
specified tolerance range (go/no-go test).

The probe passes if at least 90% of the electrodes are within the
frequency and amplitude tolerance.
