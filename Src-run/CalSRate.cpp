...
// ---------
    ...
    if( !df->openForRead( baseName + ".imec.ap.bin", S.err ) )
        goto close;
...
// ---------------------
    ...
    if( p.im.enabled ) {
        ...
        int word = p.im.imCumTypCnt[CimCfg::imSumAll] - 1;

        p.sns.imChans.uiSaveChanStr = QString::number( word );
        p.sns.imChans.saveBits.clear();
        p.sns.imChans.saveBits.resize( p.im.imCumTypCnt[CimCfg::imSumAll] );
        p.sns.imChans.saveBits.setBit( word );
    }
