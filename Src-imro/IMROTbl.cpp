...
// New codes ---------------------------------
        ...
        switch( pn.mid( 2 ).toInt() ) {
            ...
            case 1221:  // Custom layout defaults
                ...
                _zpitch     = 31;
                break;
            case 1300:  // Optopix phase1
            case 1400:  // Opto-II pre-Alpha
                _ncolhwr    = 2;
                _ncolvis    = 2;
                _col2vis_ev = {0,1};
                _col2vis_od = {0,1};
...
QString IMROTbl::default_imroLE( int type )
{
    switch( type ) {
        ...
        case 3020:
        case 3022:  return "*Default (shnk 0+1, bank 0, ref ext)";
...
// t_tech_opto_p1:
// - type 1300
// - Originally only NP1300
// - NP1400 functionally same as NP1300
// - NP1400, NP1300 use same HS
// - NP1400 requires newer OPTO firmware, but...
    ...
    const PAddr&    adr )
{
    ...
    if( prbtech == t_tech_sim || bsctech == t_tech_sim )
        ...
        else if( prbtech == t_tech_opto_p1 ) {
            msg = QString(
