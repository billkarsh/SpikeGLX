...
public:
  ...
  */
   ...
   * contidion must be removed. This can be done by re-arming the system, which
   * will drop all samples until a trigger is encounter.  Or by reading out
   ...
   *
   * \param value: enable (true) or disable (false) HS heartbeat LED
   */
   ...
   */
  NP_ErrorCode readBSCSN(unsigned char slotID, int & sn) const;
  ...
  */
   ...
   */
  NP_ErrorCode readHSSN(unsigned char slotID, signed char port, int & sn) const;
  ...
  */
  NP_ErrorCode writeBSCSN(unsigned char slotID, int sn) const;
  ...
  */
  NP_ErrorCode writeHSSN(unsigned char slotID, signed char port, int sn) const;
